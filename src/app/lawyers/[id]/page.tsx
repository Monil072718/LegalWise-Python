      ...
      <main className="pt-24 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative rounded-3xl overflow-hidden bg-white shadow-xl mb-8">
                ...
                <div className="px-8 pb-8 flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8 -mt-16 relative z-10">
                    ...
                    <div className="flex-1 text-gray-900 text-center md:text-left mt-4 md:mt-0 md:pb-2">
                        <div className="flex flex-col md:flex-row items-center gap-3 mb-2 justify-center md:justify-start flex-wrap">
                            <h1 className="text-3xl md:text-4xl font-bold">{lawyer.name}</h1>
                            {lawyer.verified && (
                                <span className="bg-blue-100 border border-blue-200 text-blue-700 py-1 px-3 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 whitespace-nowrap">
                                    <Shield className="w-3.5 h-3.5 fill-blue-700" /> Verified Partner
                                </span>
                            )}
                        </div>
                        <p className="text-blue-600 text-lg flex items-center justify-center md:justify-start gap-2 font-medium">
                             {lawyer.specialization && lawyer.specialization.join(', ')} Specialist
                        </p>
                    </div>

                    <div className="flex gap-3 md:pb-2">
                         <div className="bg-gray-50 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-900 text-center shadow-sm min-w-[100px]">
                             <div className="text-[10px] text-gray-500 uppercase font-extrabold tracking-wider mb-0.5">Experience</div>
                             <div className="font-bold text-lg leading-tight">{lawyer.experience}+ Years</div>
                         </div>
                         <div className="bg-gray-50 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-900 text-center shadow-sm min-w-[100px]">
                             <div className="text-[10px] text-gray-500 uppercase font-extrabold tracking-wider mb-0.5">Rating</div>
                             <div className="font-bold text-lg leading-tight flex items-center justify-center gap-1">
                                 {lawyer.rating} <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
                             </div>
                         </div>
                    </div>
                </div>
